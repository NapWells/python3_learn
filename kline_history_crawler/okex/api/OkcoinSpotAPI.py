...
from okex.api.HttpMD5Util import buildMySign,httpGet,httpPost
...
class OKCoinSpot:
    ...
    def kline(self,symbol,since=60):
        K_LINE = '/api/v1/trades.do'
        params = ''
        if symbol:
            params = 'symbol=%(symbol)s&since=%(since)s' % {'symbol': symbol, 'since':since}
        return httpGet(self.__url, K_LINE, params)
